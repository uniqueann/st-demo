...
def display_img():
    ...
    with st.expander("💡 使用提示"):
        ...
        ## 具体描述
        尽量详细地描述你想要生成的图像内容。比如，如果你想生成一幅日落的海滩风景，不要仅仅输入"海滩日落"，而是可以尝试输入"一个宁静的海滩上，夕阳西下，天空呈现出橙红色，海浪轻轻拍打着沙滩，远处有一艘小船"。

        ## 情感和氛围
        除了描述图像的内容，还可以加入对情感或氛围的描述，比如"温馨的"、"神秘的"、"充满活力的"等，这样可以帮助模型更好地理解你想要的风格。

        ## 风格指定
        如果你有特定的艺术风格偏好，比如"印象派"、"超现实主义"等，可以在prompt中明确指出，这样生成的图像更有可能符合你的期待。

        ## 避免模糊不清的词汇
        尽量避免使用过于抽象或模糊不清的词汇，比如"美"、"好"等，这些词汇对于模型来说难以具体化，可能会导致生成的图像与预期相差较大。

        ## 使用否定词
        如果你不希望图像中出现某些元素，可以使用否定词来排除。例如，"生成一幅海滩日落的图片，但不要有船"。

        ## 分步骤输入
        对于复杂场景，可以尝试分步骤聚输入提示词，先生成基础图像，再根据需要调整或添加细节。

        ## 尝试不同的描述方式
        有时候，即使描述的是同一个场景，不同的描述方式也会得到不同的结果。可以尝试从不同的角度或使用不同的词汇来描述，看看哪种方式能得到更满意的结果。

        ## 利用模型的特定功能
        一些模型可能提供了特定的功能或参数调整选项，比如调整生成图像的分辨率、风格强度等，合理利用这些功能也可以帮助提高生成图像的质量。
        """)
